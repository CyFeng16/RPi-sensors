...
class ADS1015(object):
    ...
    def ADS1015_SINGLE_READ(self, channel):  # Read single channel data
        ...
        Config_Set = (
            ADS_CONFIG_MODE_NOCONTINUOUS
            | ADS_CONFIG_PGA_4096  # mode：Single-shot mode or power-down state    (default)
            | ADS_CONFIG_COMP_QUE_NON  # Gain= +/- 4.096V                              (default)
            | ADS_CONFIG_COMP_NONLAT  # Disable comparator                            (default)
            | ADS_CONFIG_COMP_POL_LOW  # Nonlatching comparator                        (default)
            | ADS_CONFIG_COMP_MODE_TRADITIONAL  # Comparator polarity：Active low               (default)
            | ADS_CONFIG_DR_RATE_480  # Traditional comparator                        (default)
